    ...
    if(choose.length > 1) {
        message.channel.send("Je réfléchis :thinking:").then(result => {
            setTimeout(function () {
                ...
                if(message.member.displayName !== null && choose.includes(message.member.displayName) && choose[random] !== message.member.displayName){
                    result.react("😎");
                }
            }, 3000);
        });
        ...
        let help = new Discord.RichEmbed()
            .setColor("#FF0000")
            .setTitle('❌')
            .setDescription(config.prefix + 'choose [choix1]/[choix2]');
